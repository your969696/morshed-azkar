...
function AzkarCard({
  ...
  return (
    <motion.div
      ...
      style={{
        background: isDone ? 'rgba(0,200,150,0.025)' : '#151030',
        border: `1px solid ${isDone ? 'rgba(0,200,150,0.1)' : 'rgba(255,255,255,0.04)'}`,
        borderRadius: 10,
        position: 'relative',
        overflow: 'hidden',
    ...
    >
      ...
      <div style={{
        ...
        background: isDone
          ...
          : 'linear-gradient(90deg, transparent, rgba(240,176,64,0.25), transparent)',
      }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <button onClick={e => { e.stopPropagation(); onToggleComplete(); }}
            style={{
              ...
              border: `1.5px solid ${isCompleted ? '#00c896' : 'rgba(255,255,255,0.1)'}`,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <button onClick={e => { e.stopPropagation(); onSpeak(); }}
            style={{
              ...
              background: isSpeaking ? 'rgba(0,200,150,0.1)' : 'rgba(255,255,255,0.03)',
            }}>
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={isSpeaking ? '#00c896' : '#7a6f96'} strokeWidth="2" strokeLinecap="round">
      ...
      {total > 1 && (
        <div style={{ padding: '0 8px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
            <span style={{ fontSize: 7.5, color: isDone ? '#00c896' : 'rgba(255,255,255,0.18)', fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>
              {toArabicNum(count)}/{toArabicNum(total)}
            </span>
            <span style={{ fontSize: 7.5, color: isDone ? '#00c896' : 'rgba(255,255,255,0.18)', fontWeight: 600 }}>
              {Math.round(pct)}%
            </span>
          </div>
          <div style={{ height: 2.5, background: 'rgba(255,255,255,0.04)', borderRadius: 2, overflow: 'hidden' }}>
            <motion.div
              style={{ height: '100%', borderRadius: 2, background: isDone ? '#00c896' : 'linear-gradient(90deg, #8b5cf6, #00c896)' }}
        ...
        <div style={{ padding: '4px 8px 0' }}>
          <button onClick={e => { e.stopPropagation(); onToggleExpand(); }}
            style={{
              ...
              background: isExpanded ? 'rgba(240,176,64,0.05)' : 'rgba(255,255,255,0.02)',
              border: `1px solid ${isExpanded ? 'rgba(240,176,64,0.12)' : 'rgba(255,255,255,0.04)'}`,
              borderRadius: 7, cursor: 'pointer', color: '#f0b040', fontSize: 8.5, fontWeight: 600,
            ...
            {isExpanded && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                ...
                <div style={{ marginTop: 4, padding: '6px 8px', borderRadius: 8, background: 'rgba(139,92,246,0.03)', border: '1px solid rgba(139,92,246,0.07)' }}>
                  ...
                  {azkar.reference && (
                    <div style={{ marginTop: 5, padding: '4px 7px', background: 'rgba(240,176,64,0.03)', borderRadius: 5, border: '1px solid rgba(240,176,64,0.06)' }}>
                      <p style={{ fontSize: 8, color: '#f0b040', margin: 0, display: 'flex', alignItems: 'center', gap: 3 }}>
          ...
          {total > 1 ? (
            <motion.button onClick={handleTap} whileTap={{ scale: 0.92 }}
              ...
              style={{
                ...
                background: isDone ? 'linear-gradient(135deg, #00c896, #00a87d)' : 'linear-gradient(135deg, rgba(139,92,246,0.14), rgba(139,92,246,0.04))',
                boxShadow: isDone ? '0 2px 12px rgba(0,200,150,0.18)' : 'none',
              }}>
              {isDone
            ...
            <button onClick={handleTap}
              style={{
                ...
                background: isDone ? '#00c896' : 'rgba(139,92,246,0.1)',
              }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {isDone && (
            <motion.span initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }}
              style={{ fontSize: 9, color: '#00c896', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 2 }}>
...
export default function MorningAzkar() {
  ...
  const [completedIds, setCompletedIds] = useState(() => {
    ...
    try {
      const raw = localStorage.getItem('morningAzkarCompleted');
  ...
  const totalAzkar = morningAzkar.length;
  const completedCount = morningAzkar.filter(a => completedIds.includes(a.id)).length;
  ...
  const toggleCompleted = id => {
    setCompletedIds(prev => {
      const next = prev.includes(id) ? prev.filter(f => f !== id) : [...prev, id];
      localStorage.setItem('morningAzkarCompleted', JSON.stringify({ date: new Date().toDateString(), ids: next }));
      return next;
    });
  };

  const filtered = morningAzkar.filter(a => {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
      ...
      <div className="absolute inset-0" style={{
        background: 'radial-gradient(ellipse at 20% 20%, rgba(124,58,237,0.04) 0%, transparent 50%), radial-gradient(ellipse at 80% 80%, rgba(240,176,64,0.025) 0%, transparent 50%)',
      }} />
      ...
      <header className="relative flex-shrink-0 z-10" style={{ height: 62 }}>
        <div className="absolute inset-0" style={{
          ...
          <div className="absolute" style={{ top: -20, right: -15, width: 120, height: 120, borderRadius: '50%', background: 'radial-gradient(circle, rgba(240,176,64,0.07), transparent 70%)', pointerEvents: 'none' }} />
        </div>
        ...
        <div className="relative h-full flex items-center justify-between px-3.5">
          <div className="flex items-center" style={{ gap: 8 }}>
            <div style={{
              ...
              background: 'linear-gradient(135deg, rgba(240,176,64,0.14), rgba(240,176,64,0.03))',
              border: '1px solid rgba(240,176,64,0.1)', fontSize: 16, flexShrink: 0,
              boxShadow: '0 2px 8px rgba(240,176,64,0.06)',
            }}>🌅</div>
            <div>
              <h1 style={{ fontSize: 13, fontWeight: 800, color: '#fff', lineHeight: '15px', margin: 0, fontFamily: '"Cairo", sans-serif' }}>أذكار الصباح</h1>
          ...
          <div className="flex items-center" style={{ gap: 5 }}>
            {completedCount > 0 && (
              <span style={{
                ...
                color: allDone ? '#00c896' : '#f0b040', padding: '2px 7px', borderRadius: 7,
                background: allDone ? 'rgba(0,200,150,0.07)' : 'rgba(240,176,64,0.05)',
                border: `1px solid ${allDone ? 'rgba(0,200,150,0.1)' : 'rgba(240,176,64,0.08)'}`,
              }}>
            ...
            <button onClick={() => { const n = !soundEnabled; setSoundEnabled(n); localStorage.setItem('soundEnabled', String(n)); }}
              style={{
                ...
                background: soundEnabled ? 'rgba(0,200,150,0.07)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${soundEnabled ? 'rgba(0,200,150,0.1)' : 'rgba(255,255,255,0.04)'}`,
              }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={soundEnabled ? '#00c896' : '#7a6f96'} strokeWidth="2" strokeLinecap="round">
                <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/>
        ...
        {completedCount > 0 && (
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, background: 'rgba(255,255,255,0.03)' }}>
            <motion.div
              style={{ height: '100%', background: allDone ? '#00c896' : 'linear-gradient(90deg, #f0b040, #00c896)' }}
          ...
          return (
            <button key={f.key} onClick={() => setFilter(f.key)}
              style={{
                ...
                background: active ? 'rgba(240,176,64,0.08)' : 'transparent',
                border: `1px solid ${active ? 'rgba(240,176,64,0.14)' : 'transparent'}`,
                color: active ? '#f0b040' : 'rgba(255,255,255,0.2)',
          ...
          {allDone && filter === 'all' && (
            <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
              style={{
                ...
                background: 'rgba(0,200,150,0.03)', border: '1px solid rgba(0,200,150,0.08)',
              }}>
              <div style={{ fontSize: 24, marginBottom: 3 }}>✅</div>
              <p style={{ color: '#fff', fontWeight: 800, fontSize: 12, margin: '0 0 2px', fontFamily: '"Cairo", sans-serif' }}>ما شاء الله! أكملت أذكار الصباح</p>
